...
export class SlackFormatter {
  ...
  private formatCompact(epic: JiraIssue, subTasks: JiraIssue[], analysis: EpicAnalysis): string {
  ...
  private formatDetailed(epic: JiraIssue, subTasks: JiraIssue[], analysis: EpicAnalysis): string {
  ...
  private formatExecutive(epic: JiraIssue, subTasks: JiraIssue[], analysis: EpicAnalysis): string {
    const statusEmoji = this.getStatusEmoji(epic.fields.status.name);
    const progressBar = this.createProgressBar(analysis.progress);
  ...
  private formatTaskLine(task: JiraIssue): string {
  ...
  private getStatusEmoji(status: string): string {
    ...
    if (lower.includes("todo") || lower.includes("to do")) return "⚪";
    return "🔵";
  }
}
