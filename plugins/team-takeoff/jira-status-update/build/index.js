 ...
 */
export async function execute(context, epicKey, options = {}) {
    ...
    try {
        ...
        const epicResult = await context.mcp.callTool("leanix-atlassian", "searchJiraIssuesUsingJql", {
        ...
        const epic = epicResult.issues[0];
        // Fetch sub-tasks
        const subTasksResult = await context.mcp.callTool("leanix-atlassian", "searchJiraIssuesUsingJql", {
            cloudId: "leanix.atlassian.net",
            jql: `"Epic Link" = ${epicKey} ORDER BY status ASC, priority DESC`,
            fields: ["summary", "status", "issuetype", "assignee", "priority", "created", "updated", "description"],
            maxResults: 100,
        });
        const subTasks = subTasksResult.issues || [];
        // Analyze
