 ...
 */
export async function execute(context, prIdentifiers, options = {}) {
    ...
    try {
        ...
        if (desktopEnabled)
            notifyTypes.push("desktop");
        if (options.bell)
            notifyTypes.push("terminal");
        ...
        while (iteration < maxIterations) {
            try {
                ...
                const currentSnapshot = await retryHandler.execute(() => fetcher.fetch(watchOptions.prNumber), `Fetching PR #${watchOptions.prNumber}`);
                // Detect changes
                ...
                notifier.notifyError(`PR Watcher Error`, `Failed to monitor PR #${watchOptions.prNumber}: ${errorMessage}`, watchOptions.prNumber);
                // Check retry health
                ...
                if (health === "unhealthy") {
                    ...
                    notifier.notifyError(`PR Watcher Stopped`, `Connection unhealthy after ${retryHandler.getFailureCount()} failures. PR #${watchOptions.prNumber}`, watchOptions.prNumber);
                    break;
                }
            }
        }
        ...
        notifier.notifyError(`PR Watcher Crashed`, `Unexpected error for PR #${watchOptions.prNumber}: ${errorMessage}`, watchOptions.prNumber);
        return errorDetails;
    }
}
/**
 ...
 */
async function handleJiraTransition(snapshot, prNumber, output) {
    try {
...
function shouldStopWatching(snapshot, condition) {
    switch (condition) {
        case "checks-pass":
            return snapshot.checks.every((c) => c.conclusion === "success" || c.conclusion === "skipped" || c.conclusion === "neutral");
        case "approved":
...
function shouldAutoStop(snapshot) {
 ...
 */
async function executeMultiPR(prNumbers, options) {
    const output = [];
    ...
    while (iteration < maxIterations) {
        try {
            ...
            const allEvents = await retryHandler.execute(() => watcher.fetchAll(), "Fetching all PRs");
            ...
            notifier.notifyError(`Multi-PR Watcher Error`, `Failed to monitor PRs: ${errorMessage}`, prNumbers[0]);
            const health = retryHandler.getHealthStatus();
            if (health === "unhealthy") {
                output.push(`⚠️  Connection unhealthy. Stopping watch.`);
                // Send final error notification
                notifier.notifyError(`Multi-PR Watcher Stopped`, `Connection unhealthy. Monitoring stopped.`, prNumbers[0]);
                break;
            }
        }
    }
