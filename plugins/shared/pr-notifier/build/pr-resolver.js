...
export class PRResolver {
     ...
     */
    async resolveMultiple(identifiers) {
        const resolved = [];
     ...
     */
    async resolveTicketKey(ticketKey) {
        try {
            // Search for PRs that contain the ticket key
            // Check: branch name, title, and body
            const searchResults = execSync(`gh pr list --limit 50 --json number,title,headRefName,body --state all`, { encoding: "utf-8" });
     ...
     */
    async findAllForTicket(ticketKey) {
        try {
            const searchResults = execSync(`gh pr list --limit 50 --json number,title,headRefName,body --state all`, { encoding: "utf-8" });
            const prs = JSON.parse(searchResults);
            const normalizedKey = ticketKey.toUpperCase();
